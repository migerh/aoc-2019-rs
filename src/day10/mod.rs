...
fn is_visible(map: &Vec<Coords>, asteroid: usize, other_asteroid: usize) -> bool {
  ...
  for (i, coords) in map.iter().enumerate() {
    ...
    let is_between = (distance(coords_asteroid, *coords) + distance(coords_other, *coords) - distance(coords_asteroid, coords_other)).abs() < 1e-3;
...
fn find_best_asteroid(input: &str) -> i32 {
  ...
  println!("Best asteroid: {:?}", results[0].0);
  results[0].1 as i32
}

pub fn problem1() {
  let result = find_best_asteroid(include_str!("./data/input-1.txt"));
  // let result = find_best_asteroid(include_str!("./data/example3.txt"));
  println!("Result 10-1: {}", result);
}
...
fn cleanup_angles(angles: Vec<f64>, empty: &Vec<f64>) -> Vec<f64> {
  ...
  for e in empty {
    angles.remove_item(e);
  }

  angles
}

fn destroy_asteroids(input: &str, laser: Coords) -> Vec<Coords> {
  let mut asteroids = parse_input(input);
  let mut angles = get_all_angles(&asteroids, laser);

  asteroids.remove_item(&laser);
...
pub fn problem2() {
  ...
  println!("Result 10-2: {} ({:?})", d200.0 * 100 + d200.1, d200);
}
